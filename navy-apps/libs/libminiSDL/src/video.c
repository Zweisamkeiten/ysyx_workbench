...
void SDL_BlitSurface(SDL_Surface *src, SDL_Rect *srcrect, SDL_Surface *dst, SDL_Rect *dstrect)
{
  ...
  case 8:
    ...
    {
      s8 = (uint8_t *)((uintptr_t)src->pixels + (sx + (sy + i) * src->w) * 1);
      d8 = (uint8_t *)((uintptr_t)dst->pixels + (dx + (dy + i) * dst->w) * 1);
  ...
  case 32:
    for (uint16_t i = 0; i < h; i++)
    {
      s = (uint32_t *)((uintptr_t)src->pixels + (sx + (sy + i) * src->w) * 4);
      d = (uint32_t *)((uintptr_t)dst->pixels + (dx + (dy + i) * dst->w) * 4);
      for (uint16_t j = 0; j < w; j++)
      {
        d[j] = s[j];
      }
...
void SDL_FillRect(SDL_Surface *dst, SDL_Rect *dstrect, uint32_t color)
{
  ...
  case 8:
    for (int i = 0; i < h; i++)
    {
      p8 = (uint8_t *)((uintptr_t)dst->pixels + (x + (y + i) * dst->w) * sizeof(uint8_t));
      for (int j = 0; j < w; j++)
      {
        p8[j] = color;
      }
  ...
  case 32:
    for (int i = 0; i < h; i++)
    {
      p = (uint32_t *)((uintptr_t)dst->pixels + (x + (y + i) * dst->w) * sizeof(uint32_t));
      for (int j = 0; j < w; j++)
      {
        p[j] = color;
      }

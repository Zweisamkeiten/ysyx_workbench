...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  // INSTPAT("??????? ????? ????? 001 00000 11100 11", csrrw  , I, printf("csrrw\n"); int csr_idx = get_csr_idx(imm); R(dest) = cpu.csr[csr_idx]; cpu.csr[csr_idx] = src1);
  INSTPAT("??????? ????? ????? ??? ????? ????? ??", inv    , N, printf("123");INV(s->pc));
  INSTPAT_END();

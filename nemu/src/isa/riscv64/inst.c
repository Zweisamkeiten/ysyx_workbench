...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  // RV32I: shift bits by low-5 bits rs2; RV64I: shift bits by low-6 bits rs2;
  ...
  INSTPAT("??????? ????? ????? 000 ????? 00110 11", addiw  , I, R(dest) = SEXT(BITS(src1, 31, 0) +  BITS(imm, 31, 0), 32));
  INSTPAT("0000000 ????? ????? 001 ????? 00110 11", slliw  , I, R(dest) = SEXT(BITS(src1, 31, 0) << BITS(imm, 31, 0), 32));
  INSTPAT("0000000 ????? ????? 101 ????? 00110 11", srliw  , I, R(dest) = SEXT(BITS(src1, 31, 0) >> BITS(imm, 31, 0), 32));
  INSTPAT("0100000 ????? ????? 101 ????? 00110 11", sraiw  , I, R(dest) = SEXT((int32_t)BITS(src1, 31, 0) >> BITS(imm, 31, 0), 32));
  ...
  // RV32M
  INSTPAT("0000001 ????? ????? 000 ????? 01100 11", mul    , R, R(dest) = src1 * src2; multi_count++);
  INSTPAT("0000001 ????? ????? 001 ????? 01100 11", mulh   , R, R(dest) = (__int128)src1 * (__int128)src2 >> 64; multi_count++);
  INSTPAT("0000001 ????? ????? 010 ????? 01100 11", mulhsu , R, R(dest) = (__int128)((__int128)src1 * (__uint128_t)src2) >> 64; multi_count++);
  INSTPAT("0000001 ????? ????? 011 ????? 01100 11", mulhu  , R, R(dest) = (__uint128_t)src1 * (__uint128_t)src2 >> 64; multi_count++);
  ...
  // RV64M
  INSTPAT("0000001 ????? ????? 000 ????? 01110 11", mulw   , R, R(dest) = SEXT((BITS(src1, 31, 0) * BITS(src2, 31, 0)), 32); multi_count++);
  ...
  INSTPAT("0000000 00000 00000 000 00000 11100 11", ecall  , N, s->dnpc = NEMUINTR(11, s->pc)); // Environment call from M-mode Expection Code: 11
  INSTPAT("0000000 00001 00000 000 00000 11100 11", ebreak , N, NEMUTRAP(s->pc, R(10))); // R(10) is $a0

  // RV32/RV64 Zicsr
  INSTPAT("??????? ????? ????? 001 ????? 11100 11", csrrw  , I, R(dest) = csr(imm); csr(imm) = src1);
  INSTPAT("??????? ????? ????? 010 ????? 11100 11", csrrs  , I, R(dest) = csr(imm); csr(imm) = src1 | csr(imm));

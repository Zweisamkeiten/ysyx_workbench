...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  // lw: x[rd] = sext(M[x[rs1] + sext(offset)][31:0]); for RV64I, sign-extends 32-bit value to 64 bits.
  ...
  INSTPAT("0000000 ????? ????? 001 ????? 00100 11", slli   , I, uint64_t a = (uint64_t)(BITS(imm, 25, 20)); R(dest) = src1 << a);
  INSTPAT("0000000 ????? ????? 101 ????? 00100 11", srli   , I, R(dest) = src1 >> BITS(imm, 25, 20));
